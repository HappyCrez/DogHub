...
### `GET /services/types`
- Возвращает список доступных услуг DogHub (`id`, `name`, `price`) из таблицы `service_type`.
- Доступен без авторизации.

### `GET /services/my`
- Требует авторизации. Для обычных пользователей возвращает историю услуг только их собак, администратор видит все записи.
- Ответ: `{ services: DogServiceRow[], statusLabels: { [status]: string } }`, где `DogServiceRow` содержит `id`, `dogId`, `dogName`, `serviceTypeId`, `serviceName`, `requestedAt`, `performedAt`, `price`, `status`.

### `POST /services/book`
- Требует авторизации. Создаёт заявку на услугу для конкретной собаки.
- Тело запроса: `{ "dogId": number, "serviceTypeId": number, "requestedAt": string (ISO) }`.
- Стоимость услуги подставляется автоматически из `service_type.price` и фиксируется в `dog_service.price`.
- Статус новой заявки всегда `REQUESTED`. Владелец собаки должен совпадать с текущим пользователем (кроме администраторов).

### `DELETE /services/{id}`
- Требует авторизации. Отменяет заявку на услугу: обычный пользователь может отменить только свои заявки, администратор — любые.
- Если заявка уже выполнена или отменена ранее, сервер вернёт `409 Conflict`.
- Успешный ответ содержит обновлённую строку с полем `status = "CANCELED"`.