...
Основные параметры считываются из `Assets/.env`. Помимо настроек БД и JWT добавлены переменные для управления refresh‑сессиями:

- `ACCESS_TOKEN_LIFETIME_MINUTES` — срок жизни access‑токена.
- `REFRESH_TOKEN_LIFETIME_DAYS` — срок жизни refresh‑токена (по умолчанию 14 дней).
- `REFRESH_COOKIE_DOMAIN` — домен для HttpOnly‑cookie с refresh‑токеном. Оставьте пустым для разработки.
- `REFRESH_COOKIE_SECURE` — `true/false`, включает передачу cookie только по HTTPS.
- `CORS_ALLOWED_ORIGINS` — список разрешённых origin (через запятую). Для работы HttpOnly‑cookie необходимо указать явные адреса фронтенда и backend автоматически включит `AllowCredentials`.
...
- **Авторизация и регистрация**: хеширование паролей на клиенте (SHA‑256), хранение access‑токена в `AuthContext`/`localStorage` и автоматическое продление сессии через HttpOnly refresh‑cookie (endpoint `/auth/refresh`), централизованный logout.
